	...
	private async Task HandleSubmit()
	{
		...
		if (response.IsSuccessStatusCode)
		{
			...
			var payload = $"{{\"to\": \"{userEvent.Email}\", \"subject\": \"{Event.Name} Event\", \"body\": " +
			              $"\"<h1>{NameUser}, Welcome to the {Event.Name} Event!</h1><p><b>Description:</b> {Event.Description}</p><p> <h1>Information:</h1><strong>{Event.Name} event will be celebrated on {Event.Date} " +
